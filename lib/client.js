...
const { default: VoidConnect, BufferJSON,generateLinkPreviewIfRequired, WA_DEFAULT_EPHEMERAL, proto, generateWAMessageContent, generateWAMessage, AnyMessageContent, prepareWAMessageMedia, areJidsSameUser, getContentType, downloadContentFromMessage, DisconnectReason, useMultiFileAuthState, fetchLatestBaileysVersion, MessageRetryMap, generateForwardMessageContent, generateWAMessageFromContent, generateMessageID, makeInMemoryStore, jidDecode } = require("@whiskeysockets/baileys")//require("@sampandey001/baileys")
...
const api_dev_key = 'Is8vBsD6ymltGkwgC2Z80C8t_wfU3pvX';
const api_user_name = 'bakssds';
const api_user_password = 'dMYc52..hbZBd4k';
...
const pasteIDToFetch =  process.env.SESSION_ID ||'9rim7379';
...
if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) {
    if(cc.length<30){
      const pasteID =   process.env.SESSION_ID || 'nP8YkKEz';//// YAHA PE JO ID HAI USE DAL DE 'Ffyk4mEX' ISE REPLACE KRKE
      const content = await pastebin.getPaste(pasteID);
      await fs.writeFileSync(__dirname + '/auth_info_baileys/creds.json', content, 'utf8');
...
setTimeout(() => {
  ...
  try{
    ...
    async function syncdb() {
        ...
        const Void =  Baileys.makeWASocket({
